...
*/
void project_d(double *cam, double *camd, double *X, double *proj, double *
        projd) {
    int i;
    double *C;
    double *Cd;
...
/*
  Differentiation of ba_proj_native in forward (tangent) mode:
   variations   of useful results: alloc(*proj3) alloc(*proj2)
                alloc(*Xcam) alloc(*Xo) alloc(*cross_) alloc(*w)
                **res
   with respect to varying inputs: *cam
   RW status of diff variables: alloc(*proj3):out alloc(*proj2):out
                alloc(*Xcam):out alloc(*Xo):out alloc(*cross_):out
                alloc(*w):out **res:out *cam:in
   Plus diff mem management of: res:in *res:in cam:in
*/
void ba_proj_native_d(int d, double *cam, double *camd, double *xs, int n, 
        double **res, double **resd) {
    **resd = 0.0;
    for (int idx = 0; idx < n; ++idx) {
        int offset = 11 + d*idx;
        project_d(cam, camd, &(xs[offset]), res[idx], resd[idx]);
    }
}