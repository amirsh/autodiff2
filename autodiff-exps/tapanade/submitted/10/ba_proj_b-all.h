...
void cross_c(double *a, double *b, double *out) {
...
double dot_prod_c(int n, double *x, double *y) {
    double res = 0;
    for (int i = 0; i < n; ++i)
        res += x[i]*y[i];
    return res;
...
void ba_rod_single_c(double *rot, double *pt, double *rotatedPt) {
    int i;
    double sqtheta, theta, costheta, sintheta, theta_inverse, tmp;
    double *w;
    double result1;
...
void radial_distort_c(double *rad_params, double *proj, double *res) {
    double rsq, L;
    rsq = sqsum_c(2, proj);
    L = 1 + rad_params[0]*rsq + rad_params[1]*rsq*rsq;
    res[0] = proj[0]*L;
...
/*
  Differentiation of project in reverse (adjoint) mode:
   ...
   with respect to varying inputs: alloc(*proj3) alloc(*proj2)
                alloc(*Xcam) alloc(*Xo) alloc(*cross_) alloc(*w)
                *cam *proj
   Plus diff mem management of: cam:in proj:in
*/
void project_b(double *cam, double *camb, double *X, double *proj, double *
        projb) {
    int i;
    double *C;
    double *Cb;
    ...
    for (i = 0; i < 3; ++i)
        Xo[i] = X[i] - C[i];
    ba_rod_single_c(&(cam[0]), Xo, Xcam);
    proj2[0] = Xcam[0]/Xcam[2];
    proj2[1] = Xcam[1]/Xcam[2];
    radial_distort_c(&(cam[9]), proj2, proj3);
    for (i = 1; i > -1; --i) {
...
void project_c(double *cam, double *X, double *proj) {
    int i;
    double *C;
    double *Xo;
    Xo = (double *)malloc(sizeof(double)*3);
...
/*
  Differentiation of ba_proj_native in reverse (adjoint) mode:
   ...
   RW status of diff variables: alloc(*proj3):in-out alloc(*proj2):in-out
                ...
                alloc(*w):in-out **res:in-out *cam:incr
   Plus diff mem management of: res:in *res:in cam:in
*/
void ba_proj_native_b(int d, double *cam, double *camb, double *xs, int n, 
        double **res, double **resb) {
    for (int idx = 0; idx < n; ++idx) {
        int offset = 11 + d*idx;
        project_c(cam, &(xs[offset]), res[idx]);
        pushinteger4(offset);
    }
    for (int idx = n-1; idx > -1; --idx) {
        int offset;
        popinteger4(&offset);
        project_b(cam, camb, &(xs[offset]), res[idx], resb[idx]);
    }
}
